...
void callback(const ImageConstPtr &intensity, const ImageConstPtr &distance, image_transport::Publisher &pub)
{
    ...
    try
    {
        intensity_ptr = cv_bridge::toCvCopy(intensity, image_encodings::MONO8);
        distance_ptr = cv_bridge::toCvCopy(distance, image_encodings::MONO8);
        corrected = cv_bridge::toCvCopy(distance, image_encodings::TYPE_64FC1);
    }
